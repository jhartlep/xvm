 ...
 */
{
  "battleLoading": {
    ...
    "formatRightNick": "{{name}}",
    // Display format for the left panel (macros allowed, see readme-en.txt).
    // Формат отображения для левой панели (допускаются макроподстановки, см. readme-ru.txt).
    "formatLeftVehicle": "{{vehicle}}<font face='Lucida Console' size='11'> <font color='{{c:kb}}'>{{kb%2d}}k</font> <font color='{{c:wn8}}'>{{wn8%4s}}</font> <font color='{{c:rating}}'>{{rating}}</font> <font color='{{c:t-rating}}' size='9'>{{t-rating}}</font></font>",
    // Display format for the right panel (macros allowed, see readme-en.txt).
    // Формат отображения для правой панели (допускаются макроподстановки, см. readme-ru.txt).
    "formatRightVehicle": "<font face='Lucida Console' size='11'><font color='{{c:t-rating}}' size='9'>{{t-rating}}</font> <font color='{{c:rating}}'>{{rating}}</font> <font color='{{c:wn8}}'>{{wn8%4s}}</font> <font color='{{c:kb}}'>{{kb%2d}}k</font> </font>{{vehicle}}"
  }
}
